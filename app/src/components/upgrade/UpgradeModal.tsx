...
const UPGRADE_PLANS = [
  {
    id: 'free', name: 'Free', tagline: 'Use Grain for personal use; free forever.',
    price: '$0', unit: false, monthly: 'Free forever',
    btn: { label: 'Current plan', kind: 'disabled' as const },
    features: ['Personal account', 'Unlimited meetings', '45-min meeting limit', '30-day history limit', 'Desktop capture & bots', 'MCP access'],
  },
  {
    id: 'pro', name: 'Pro', tagline: 'Use Grain personally with no limits.',
    price: '$14', unit: true, monthly: '$18 billed monthly',
    btn: { label: 'Upgrade', kind: 'neutral' as const },
    features: ['Personal account only', 'No meeting length cap', 'Unlimited history', 'No upload cap', 'MCP access'],
  },
  {
    id: 'business', name: 'Business', tagline: "Power your organization's AI with shared meeting intelligence.",
    price: '$34', unit: true, monthly: '$42 billed monthly',
    btn: { label: 'Upgrade', kind: 'primary' as const },
    badge: '14 days left in trial', highlight: true,
    features: ['Organization workspace', 'No meeting length cap', 'Unlimited history', 'No upload cap', 'API & Workspace integrations'],
  },
];

function UpPlan({ plan, reactivate = false, showTrialBadge = true }: { plan: typeof UPGRADE_PLANS[number]; reactivate?: boolean; showTrialBadge?: boolean }) {
  ...
  return (
    <div className={`up-plan ${plan.highlight ? 'is-highlight' : ''}`}>
      ...
      <button className={`up-plan__btn up-plan__btn--${plan.btn.kind}`} disabled={plan.btn.kind === 'disabled'}>{btnLabel}</button>
...
interface UpgradePlanModalProps {
  open: boolean; onClose?: () => void; reactivate?: boolean; showTrialBadge?: boolean; onTalkToSales?: () => void;
}
export function UpgradePlanModal({ open, onClose, reactivate = false, showTrialBadge = true, onTalkToSales }: UpgradePlanModalProps) {
  return (
...
function CoField({ label, children, htmlFor }: { label: string; children: React.ReactNode; htmlFor?: string }) {
  return (
...
interface CheckoutModalProps {
  open: boolean; onClose?: () => void; onBack?: () => void; billingEmail?: string; onPaid?: () => void;
}
export function CheckoutModal({ open, onClose, onBack, billingEmail = '', onPaid }: CheckoutModalProps) {
  ...
  useEffect(() => {
    if (open) { setAgreed(false); setMethod('card'); setPromoOpen(false); setPromo(''); setEmail(billingEmail); }
  }, [open, billingEmail]);
        ...
        <div className="co__body">
          <div className="co__form">
            <div className="co-methods" role="tablist" aria-label="Payment method">
              <button role="tab" aria-selected={method === 'card'} className={`co-method ${method === 'card' ? 'is-active' : ''}`} onClick={() => setMethod('card')}>
                <Icon name="creditCard" size={20} />
                <span className="co-method__label">Card</span>
              </button>
              <button role="tab" aria-selected={method === 'bank'} className={`co-method ${method === 'bank' ? 'is-active' : ''}`} onClick={() => setMethod('bank')}>
          ...
          <aside className="co__summary">
            ...
            <div className="co-sum__card">
              ...
              <div className="co-sum__line"><span>Annual plan</span><span>${BUSINESS_ANNUAL.toFixed(2)}</span></div>
              <div className="co-sum__line"><span>Billed today</span><span>${BUSINESS_ANNUAL.toFixed(2)}</span></div>
              <div className="co-sum__rule" />
              <div className="co-sum__line co-sum__line--total"><span>Total</span><span>${BUSINESS_ANNUAL.toFixed(2)}</span></div>
            </div>
