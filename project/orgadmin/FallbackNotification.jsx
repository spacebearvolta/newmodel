// FallbackNotification.jsx — macOS notification card shown when fallback fires.
...
function FallbackNotification({ variant }) {
  // variant: 'bot-failed' | 'desktop-failed'
  const botIcon = (
  ...
  const cfg = variant === 'desktop-failed'
    ? {
        actionLabel: 'Starting bot capture in 5…',
        footer: 'Starting because desktop capture failed',
        icon: botIcon,
      }
    : {
        actionLabel: 'Starting desktop capture in 5…',
  ...
  return (
    <div className="notif">
      ...
      <div className="notif__action">
        <span className="notif__action-icon" aria-hidden="true">
          {cfg.icon}
        </span>
        <span className="notif__action-label">{cfg.actionLabel}</span>
        <button type="button" className="notif__cancel" aria-label="Cancel">
          <span />
        </button>
      </div>
...
function FallbackNotificationArtboard() {
  return (
      ...
      <div className="artboard__frame">
        <div className="artboard__row">
          <div className="artboard__variant">
            ...
            <div className="artboard__stage">
              <FallbackNotification variant="bot-failed" />
            </div>
          </div>
          <div className="artboard__variant">
            <p className="artboard__caption">When desktop capture fails → bot takes over</p>
            <div className="artboard__stage">
              <FallbackNotification variant="desktop-failed" />
            </div>
          </div>
        </div>
      </div>
...
Object.assign(window, { FallbackNotification, FallbackNotificationArtboard });
