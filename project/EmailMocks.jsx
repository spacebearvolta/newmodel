// EmailMocks.jsx — Designed email mockups for the post-trial hooks.
...
function EmailFrame({ from, fromAddr, subject, children }) {
  return (
...
function TrialEndedEmail({ orgName = 'Acme', graceDays = 30 }) {
  return (
    <EmailFrame from="Grain" fromAddr="hello@grain.com" subject={`Your Grain Business trial has ended — your meetings are saved for ${graceDays} days`}>
      <EmailShell>
        <div className="em-icon em-icon--warm"><i data-lucide="clock" /></div>
        ...
        <div className="em-callout">
          <span className="em-callout__icon"><i data-lucide="shield" /></span>
          <div>
...
function TeammateEmail({ orgName = 'Acme', daysLeft = 8, teammate = 'Maya Chen' }) {
  ...
  return (
    <EmailFrame from="Grain" fromAddr="hello@grain.com" subject={`${first} had their first meeting captured in ${orgName}`}>
      <EmailShell>
        <div className="em-eyebrow"><i data-lucide="sparkles" /> Your trial is working</div>
        ...
        <div className="em-mtg">
          <span className="em-mtg__thumb"><i data-lucide="video" /></span>
...
function EmailMocks({ email = 'trial-ended', orgName = 'Acme', graceDays = 30, daysLeft = 8 }) {
  React.useEffect(() => { if (window.lucide) window.lucide.createIcons(); });
  return (
    <div className="em-stage" data-screen-label={`Email — ${email}`}>
      {email === 'trial-ended'
...
Object.assign(window, { EmailMocks });
